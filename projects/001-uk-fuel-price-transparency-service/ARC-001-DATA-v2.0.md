...
> **Template Origin**: Official | **ArcKit Version**: 4.6.3-rc.1 | **Command**: `/arckit.data-model`

## Document Control

| Field | Value |
|-------|-------|
| **Document ID** | ARC-001-DATA-v2.0 |
...
**Key architectural change from v1.0**: The Organisation entity has been removed — retailer registration and account management is handled by VE3 Global's platform via GOV.UK One Login. The CMA service does not store retailer PII (contact details). This significantly simplifies the CMA's GDPR obligations.

### Model Statistics

- **Total Entities**: 8 entities defined (E-001 through E-008)
- **Total Attributes**: 78 attributes across all entities
...
erDiagram
    ...
    FORECOURT {
        string forecourt_id PK "VE3 node_id GB-NNNNN"
        string trading_name "Forecourt display name"
        string brand_name "Brand (Shell, BP, etc)"
        json address "VE3 location object"
        decimal latitude "WGS84 latitude"
        decimal longitude "WGS84 longitude"
        geometry geom "PostGIS Point(4326)"
        array fuel_types "Fuel types sold"
        array amenities "Forecourt amenities"
        json opening_times "Operating hours"
    ...
    CURRENT_PRICE {
        string forecourt_id FK "VE3 node_id"
        string fuel_type FK "E10 E5 B7 SDV"
        decimal price_gbp "GBP per litre 3dp"
    ...
    COMPLIANCE_RECORD {
        string forecourt_id PK "VE3 node_id"
    ...
    ENFORCEMENT_ACTION {
        ...
        string forecourt_id FK "Target forecourt"
    ...
    FUEL_TYPE {
        string fuel_code PK "E10 E5 B7 SDV"
...
**Description**: A physical fuel retail location synced from the VE3 Global Fuel Finder Public API. Each forecourt represents a petrol filling station with a unique VE3-assigned identifier.
...
**Business Context**: The forecourt is the central entity — every price, compliance assessment, and enforcement action relates to a specific forecourt. Data is sourced entirely from the VE3 Fuel Finder Public API (`GET /api/v1/pfs`). CMA enriches with PostGIS geospatial index for proximity search.
...
- **Source of Truth**: VE3 Global Fuel Finder API (CMA holds a synchronised copy)

**Data Classification**: OFFICIAL — PUBLIC (published as open data)

**Volume Estimates**:

- **Initial Volume**: ~8,500 forecourts at go-live
- **Growth Rate**: +50-100 per year (new builds, minus closures)
- **Peak Volume**: ~10,000 forecourts at Year 3
...
- **Deletion Policy**: Forecourt records are never deleted; status set to 'closed' if deregistered

#### Attributes

| Attribute | Type | Required | PII | Description | Validation Rules | Default | Source Req |
|-----------|------|----------|-----|-------------|------------------|---------|------------|
| forecourt_id | VARCHAR(8) | Yes | No | VE3 node_id (e.g., `GB-12345`) | Pattern: `GB-\d{5}` | From VE3 API | BR-008 |
| trading_name | VARCHAR(255) | Yes | No | Forecourt display name | Non-empty | From VE3 API | BR-008 |
| brand_name | VARCHAR(255) | No | No | Brand (Shell, BP, Tesco, etc.) | None | NULL | BR-008 |
| address | JSONB | Yes | No | Full location object from VE3 | Valid JSON | From VE3 API | BR-008 |
| latitude | DECIMAL(10,7) | Yes | No | WGS84 latitude | Range: 49.0–61.0 | From VE3 API | FR-002 |
| longitude | DECIMAL(10,7) | Yes | No | WGS84 longitude | Range: -8.0–2.0 | From VE3 API | FR-002 |
| geom | GEOMETRY(Point, 4326) | Yes | No | PostGIS point for spatial queries | Valid WGS84 point | Computed from lat/lng | FR-002 |
| fuel_types | TEXT[] | Yes | No | Array of fuel type codes | Values in [E10, E5, B7, SDV] | From VE3 API | BR-008 |
| amenities | TEXT[] | No | No | Forecourt amenities list | None | From VE3 API | BR-008 |
| opening_times | JSONB | No | No | Operating hours | Valid JSON | From VE3 API | BR-008 |
...
**Outgoing**:
- E-001 → E-002 (CurrentPrice): One-to-many — a forecourt has current prices for each fuel type
...
**Description**: The current published fuel price for a specific fuel type at a forecourt, synced from VE3 API and enriched with CMA data quality scoring.
...
**Business Context**: This is the primary query entity for citizen fuel price search. Composite primary key of forecourt_id + fuel_type ensures one current price per fuel grade per forecourt. Prices arrive from VE3 in GBP (3 decimal places); CMA derives PPL (pence per litre) for display.

**Data Classification**: OFFICIAL — PUBLIC

**Volume Estimates**:

- **Initial Volume**: ~25,000 records (8,500 forecourts x ~3 fuel types average)
...
| Attribute | Type | Required | PII | Description | Validation Rules | Default | Source Req |
|-----------|------|----------|-----|-------------|------------------|---------|------------|
| forecourt_id | VARCHAR(8) | Yes | No | VE3 node_id | FK to E-001 | From VE3 API | BR-008 |
| fuel_type | VARCHAR(4) | Yes | No | Fuel type code | FK to E-008 [E10, E5, B7, SDV] | From VE3 API | BR-008 |
| price_gbp | DECIMAL(6,3) | Yes | No | Price in GBP per litre (3dp) | Range: 0.500–5.000 | From VE3 API | BR-008 |
| price_ppl | DECIMAL(5,1) | Yes | No | Pence per litre (1dp display) | Computed: price_gbp x 100 | Computed | FR-002 |
| price_last_updated | TIMESTAMPTZ | Yes | No | When price changed at forecourt | ISO 8601 | From VE3 API | BR-008 |
| price_change_effective | TIMESTAMPTZ | No | No | When price change took effect | ISO 8601 | From VE3 API | BR-008 |
| synced_at | TIMESTAMPTZ | Yes | No | When synced from VE3 API | ISO 8601 | NOW() | FR-001 |
...
**Description**: Append-only historical record of all fuel prices observed from the VE3 API. Never overwritten — captures every price change for trend analysis, policy reporting, and enforcement evidence.
...
**Business Context**: Critical for three use cases: (1) DESNZ policy analysis of fuel price trends, (2) CMA enforcement evidence chain, (3) enhanced API historical price queries for third-party consumers. Partitioned by month for query performance at scale.
...
- **Growth Rate**: ~150,000 records/day (Year 1), ~300,000/day (Year 3)
- **Peak Volume**: ~165M records at Year 3
...
| forecourt_id | VARCHAR(8) | Yes | No | VE3 node_id | FK to E-001 | From sync | BR-008 |
| fuel_type | VARCHAR(4) | Yes | No | Fuel type code | FK to E-008 | From sync | BR-008 |
| price_gbp | DECIMAL(6,3) | Yes | No | Price in GBP per litre | Range: 0.500–5.000 | From VE3 API | BR-008 |
| price_last_updated | TIMESTAMPTZ | Yes | No | When price changed (from VE3) | ISO 8601 | From VE3 API | FR-015 |
...
**Description**: CMA-computed compliance status for each forecourt, derived from price submission patterns observed via the VE3 API. Updated on each sync cycle.
...
- **Initial Volume**: ~8,500 records (one per forecourt)
- **Growth Rate**: Proportional to forecourt growth
...
- **Deletion Policy**: Soft delete (status → 'archived') when forecourt deregistered

#### Attributes

| Attribute | Type | Required | PII | Description | Validation Rules | Default | Source Req |
|-----------|------|----------|-----|-------------|------------------|---------|------------|
| forecourt_id | VARCHAR(8) | Yes | No | VE3 node_id | FK to E-001, PK | From sync | BR-004 |
...
| forecourt_id | VARCHAR(8) | Yes | No | Target forecourt | FK to E-001 | From officer | BR-004 |
...
**Description**: Tracks the state of each data synchronisation batch from the VE3 Fuel Finder API. Enables incremental sync, failure recovery, and operational monitoring.
...
**Business Context**: Operational entity for managing the VE3 API sync pipeline. Records each sync batch (full or incremental), its status, records processed, and the `last_effective_timestamp` used for the next incremental sync. Critical for debugging sync failures and monitoring data freshness.
...
**Description**: Reference data mapping VE3 API fuel type codes to consumer-friendly display names and descriptions.
...
**Business Context**: Static reference table seeded at deployment. Maps the four VE3 API fuel codes (E10, E5, B7, SDV) to names citizens understand (Unleaded, Super Unleaded, Diesel, Premium Diesel). Also stores VE3 alias mappings.

**Data Classification**: OFFICIAL — PUBLIC

**Volume Estimates**: 4 records (static)
...
| Attribute | Type | Required | PII | Description | Validation Rules | Default | Source Req |
|-----------|------|----------|-----|-------------|------------------|---------|------------|
| fuel_code | VARCHAR(4) | Yes | No | VE3 API code | PK, Enum: E10, E5, B7, SDV | None | BR-008 |
...
| fuel_code | display_name | description | ve3_aliases |
|-----------|-------------|-------------|-------------|
| E10 | Unleaded | Standard unleaded petrol (10% ethanol) | {UNLEADED} |
| E5 | Super Unleaded | Premium unleaded petrol (5% ethanol) | {PREMIUM_UNLEADED} |
| B7 | Diesel | Standard diesel (7% biodiesel) | {DIESEL, B7_STANDARD} |
...
| Entity | VE3 API Sync | Citizen Web | Enhanced API | Enforcement Dashboard | Policy Reports | Batch Jobs |
|--------|-------------|-------------|-------------|----------------------|----------------|------------|
...
**Notes**:
- **VE3 API Sync** is the only component that writes price data (E-002, E-003)
...
#### INT-001: VE3 Global Fuel Finder Public API (Primary)

**Source System**: VE3 Global Fuel Finder API (api.fuelfinder.service.gov.uk)

**Integration Type**: Scheduled REST API polling

**Entities Affected**:

| Source Endpoint | Target Entity | Update Frequency | Data Quality SLA |
|-----------------|---------------|------------------|------------------|
| `GET /api/v1/pfs` | E-001 (Forecourt) | Every 1 hour | 99% completeness |
| `GET /api/v1/pfs/fuel-prices` | E-002 (CurrentPrice), E-003 (PriceHistory) | Every 15 minutes | ≤ 20 min freshness |

**Data Mapping**:

| VE3 Field | VE3 Type | Target Entity | Target Attribute | Transformation |
|-----------|----------|---------------|------------------|----------------|
| node_id | String | E-001 | forecourt_id | Direct mapping |
| trading_name | String | E-001 | trading_name | Direct mapping |
| brand_name | String | E-001 | brand_name | Direct mapping |
| location.coordinates | Array | E-001 | latitude, longitude, geom | Extract lat/lng; compute PostGIS point |
| fuel_types | Array | E-001 | fuel_types | Direct mapping |
| price | Decimal | E-002 | price_gbp | Direct mapping (GBP 3dp) |
| price | Decimal | E-002 | price_ppl | Compute: price x 100, round to 1dp |
| fuel_type | String | E-002 | fuel_type | Map aliases to canonical codes |
...
|--------|------------------|-----------|---------------------|
| E-001: Forecourt | VE3 Global API | VE3 is statutory platform | VE3 always wins |
| E-002: CurrentPrice | VE3 Global API | VE3 publishes official prices | VE3 always wins |
...
| E-008: FuelType | VE3 API specification | VE3 defines fuel codes | VE3 spec authoritative |
...
The CMA service stores no personally identifiable information. Retailer contact details (names, emails, phone numbers) are managed by VE3 Global's platform via GOV.UK One Login. The CMA service references forecourts by VE3 node_id only.
...
|--------|------------------|-------------------|-------|-------------|-----------------|
| E-001: Forecourt | Indefinite | N/A | Indefinite | Public record | Status → 'closed' |
...
|--------|-----------|-----------------|-------------------|-------|
| E-001 | latitude, longitude | 99.9% (from VE3 API) | Cross-reference with OS Places | CMA Digital |
| E-002 | price_gbp | 100% (from VE3 API) | VE3 validation at source | VE3 Global |
...
|--------|----------------|--------|-------|
| E-001 | forecourt_id, trading_name, latitude, longitude, fuel_types | 100% | CMA Digital |
...
|--------|------------|---------------------|
| E-001 | forecourt_id (from VE3) | VE3 assigns unique GB-NNNNN; CMA trusts source |
...
- **JSONB**: Native support for VE3 API JSON structures (address, amenities, opening_times, evidence_summary)
- **Partitioning**: Declarative table partitioning for PriceHistory (monthly partitions, ~55M rows/year)
...
**Versioning**: Semantic versioning (V2.0.0 for this data model release)

**Zero-Downtime Migrations**: Additive-only changes where possible; blue-green for breaking changes
...
- **Volume Testing**: Use VE3 API test environment (sandbox) for integration testing
...
| Term | Definition |
|------|-----------|
| B7 | Standard diesel (7% biodiesel) — VE3 API fuel code |
| E10 | Standard unleaded petrol (10% ethanol) — VE3 API fuel code |
| E5 | Super unleaded petrol (5% ethanol) — VE3 API fuel code |
...
| PPL | Pence per litre — UK standard fuel price display unit |
| SDV | Premium diesel (super diesel variant) — VE3 API fuel code |
...
| VE3 Global | Technology company operating the Fuel Finder API under DESNZ contract |
...
**Generated on**: 2026-04-05 10:00 GMT
**ArcKit Version**: 4.6.3-rc.1
**Project**: UK Fuel Price Transparency Service (Project 001)
**AI Model**: claude-opus-4-6
**Generation Context**: Data model v2.0 — major revision aligned with ARC-001-REQ-v3.0 architecture pivot to consume VE3 Global Fuel Finder API. Source documents: ARC-001-REQ-v3.0, ARC-001-STKE-v1.0, ARC-000-PRIN-v1.0, ARC-001-RSCH-v4.1, ARC-001-DPIA-v1.0.
